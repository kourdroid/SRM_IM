    ...
    async getIncidents(limit = 20, lastCreatedAt?: string, filters?: IncidentFilters): Promise<Incident[]> {
        ...
        if (filters) {
            ...
            if (filters.search && filters.search.trim() !== '') {
                // Sanitize input to prevent PostgREST query injection via the .or() method string parsing.
                // We strip characters that could be interpreted as PostgREST operators or separators.
